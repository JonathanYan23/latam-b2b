...
export default async function RetailerMessagesPage({
  searchParams,
}: {
  ...
  const messages = await db.message.findMany({
    ...
    take: 500,
    select: { wholesalerId: true, body: true, createdAt: true, senderId: true, readAt: true },
  });

  // 按供应商聚合会话
  const conv = new Map<string, { unread: number; last: string; at: Date }>();
  for (const m of messages) {
    const key = m.wholesalerId;
    if (!key) continue;
  ...
  const wsIds = [...conv.keys()];
  const wholesalers = wsIds.length
    ? await db.wholesaler.findMany({
        where: { id: { in: wsIds } },
        select: { id: true, business: { select: { tradeName: true } } },
      })
    : [];
  const wsName = new Map(wholesalers.map((w) => [w.id, w.business.tradeName ?? ""]));

  let rows = [...conv.entries()].sort((a, b) => b[1].at.getTime() - a[1].at.getTime());
  if (query) {
    rows = rows.filter(([wsId]) =>
      (wsName.get(wsId) ?? "").toLowerCase().includes(query),
    );
  }
  ...
  return (
    <div className="mx-auto max-w-2xl animate-fade-up">
      ...
      <form action="/retailer/messages" method="get" className="mt-5 flex max-w-sm items-center gap-2">
      ...
      {rows.length === 0 ? (
        ...
        <div className="card mt-6 divide-y divide-[var(--color-line-2)]">
          {rows.map(([wsId, c]) => (
            <Link
              key={wsId}
              href={`/retailer/suppliers/${wsId}/chat`}
