// =============================================================
// Diccionario en español（面向拉美市场）
// =============================================================

const es = {
  common: {
    selected: "Seleccionados",
    chat: "Chat",
    itemNo: "Código",
    brand: "Latam B2B",
    signIn: "Iniciar sesión",
    signOut: "Cerrar sesión",
    getStarted: "Empieza gratis",
    register: "Registrarse",
    cancel: "Cancelar",
    save: "Guardar",
    saving: "Guardando…",
    back: "Volver",
    backToHome: "Volver al inicio",
    viewAll: "Ver todo",
    view: "Ver",
    manage: "Gestionar",
    edit: "Editar",
    remove: "Eliminar",
    update: "Actualizar",
    search: "Buscar",
    all: "Todos",
    exportPdf: "Exportar PDF",
    none: "—",
    loading: "Cargando…",
    total: "Total",
    date: "Fecha",
    amount: "Monto",
    status: "Estado",
    actions: "Acciones",
    optional: "Opcional",
    more: "más",
    contactPerson: "Persona de contacto",
    inStock: "Disponible",
    lowStock: "Stock bajo",
    outOfStock: "Agotado",
    units: "unidades",
    product: "Producto",
    products: "Productos",
    supplier: "Proveedor",
    customer: "Cliente",
    price: "Precio",
    moq: "Cant. mín.",
    quantity: "Cant.",
    subtotal: "Subtotal",
    discount: "Descuento",
    shipping: "Envío",
    grandTotal: "Total general",
    terms: "Términos",
    termsNetDay: "Crédito Net {n} días",
    termsCodFull: "Contado · pago total",
    removeOne: "Reducir cantidad",
    unit: "unidad",
    zoomHint: "Toca la imagen para ampliar",
    pack: "Caja",
    notes: "Notas",
    noResults: "No se encontraron resultados",
    goToPortal: "Ir al portal",
    page: "Página",
    of: "de",
  },

  landing: {
    tagline: "Comercio mayorista B2B para LatAm",
    title:
      "Plataforma B2B integral que conecta mayoristas y compradores de Latinoamérica",
    subtitle: "Elige tu rol y entra a tu portal con un clic",
    retailerTitle: "Soy minorista",
    retailerDesc: "Encuentra proveedores, compara precios, compra y gestiona cuentas",
    retailerCta: "Ir a comprar",
    wholesalerTitle: "Soy mayorista",
    wholesalerDesc: "Vende productos, gestiona stock, capta clientes y recibe pedidos",
    wholesalerCta: "Ir a vender",
    valueRow: "Precios flexibles · Plazos de crédito · Multimoneda e idiomas · Cobertura LatAm",
    valueSub:
      "Precios públicos / de cliente / a medida · MOQ, plazos y cuentas · Pagos y logística locales",
    feat1Title: "Precios flexibles",
    feat1Desc: "Precios públicos, solo para clientes y a medida.",
    feat2Title: "Plazos de crédito",
    feat2Desc: "MOQ, plazos, saldos e historial de pedidos.",
    feat3Title: "Hecho para LatAm",
    feat3Desc: "Multipaís, multimoneda y multiidioma.",
    footerTag: "Marketplace · Gestión · Logística · Abastecimiento",
  },

  auth: {
    backToHome: "Volver al inicio",
    createAccount: "Crea tu cuenta",
    welcomeBack: "Bienvenido de nuevo",
    registerDesc: "Configura tu negocio en la plataforma.",
    loginDesc: "Inicia sesión en tu portal.",
    yourName: "Tu nombre",
    businessName: "Nombre del negocio",
    iAmA: "Soy…",
    retailerRole: "Minorista",
    retailerRoleDesc: "Compro al por mayor",
    wholesalerRole: "Mayorista",
    wholesalerRoleDesc: "Vendo al por mayor",
    email: "Correo electrónico",
    password: "Contraseña",
    passwordHint: "Al menos 6 caracteres",
    createBtn: "Crear cuenta",
    signInBtn: "Iniciar sesión",
    pleaseWait: "Espera…",
    termsNote: "Al continuar aceptas los términos de servicio de la plataforma.",
    errName: "Por favor ingresa tu nombre",
    errBusiness: "Por favor ingresa el nombre de tu negocio",
    errEmail: "Ingresa un correo válido",
    errPassword: "La contraseña debe tener al menos 6 caracteres",
    errInvalidForm: "Datos de formulario inválidos",
    errExists: "Ya existe una cuenta con este correo",
    errRegister: "El registro falló. Inténtalo de nuevo.",
    errInvalid: "Correo o contraseña incorrectos",
    country: "País",
    chooseCountry: "Selecciona tu país",
    currencyNote: "Moneda de la cuenta: {symbol} ({code})",
    sendCode: "Enviar código",
    resendIn: "Reenviar en {s}s",
    codeSent: "Enviamos un código de 6 dígitos a {email}",
    codePlaceholder: "Código de 6 dígitos",
    errCountry: "Selecciona tu país",
    errCode: "Ingresa el código de verificación",
    errCodeInvalid: "El código no es válido o caducó",
    errMailFailed: "No pudimos enviar el correo. Revisa tu dirección e inténtalo de nuevo",
    errEmailTaken: "Este correo ya está registrado — inicia sesión",
    devCodeNote: "Modo desarrollo (sin servicio de correo): tu código es {code}",
  },

  nav: {
    home: "Inicio",
    browse: "Explorar",
    orders: "Pedidos",
    suppliers: "Mis proveedores",
    discover: "Buscar proveedores",
    account: "Cuenta",
    products: "Productos",
    customers: "Clientes",
    messages: "Mensajes",
    searchProducts: "Buscar productos / categorías…",
    messagesHint: "Todas las conversaciones aquí — toca para responder",
    noMessages: "Aún no hay mensajes",
    searchMessages: "Buscar conversaciones…",

    retailerBrand: "Portal Minorista",
    wholesalerBrand: "Portal Mayorista",
  },

  security: {
    title: "Cuenta y seguridad",
    desc: "Gestiona tus credenciales y ajustes de seguridad.",
    profile: "Perfil",
    business: "Negocio",
    accountCurrency: "Moneda de cuenta",
    changePassword: "Cambiar contraseña",
    currentPassword: "Contraseña actual",
    newPassword: "Nueva contraseña",
    confirmPassword: "Confirmar nueva contraseña",
    passwordChanged: "Contraseña actualizada",
    passwordTooShort: "La nueva contraseña debe tener al menos 6 caracteres",
    wrongPassword: "La contraseña actual es incorrecta",
    mismatch: "Las contraseñas no coinciden",
    twoFactor: "Autenticación en dos pasos (2FA)",
    twoFactorDesc: "Requiere un código único al iniciar sesión para mayor protección.",
    comingSoon: "Próximamente",
    phoneBinding: "Número de teléfono",
    phoneDesc: "Vincula tu teléfono para recuperación y alertas de seguridad.",
    menuAccount: "Cuenta y pagos",
    menuSecurity: "Seguridad de cuenta",
  },

  retailerHome: {
    welcome: "Bienvenido de nuevo",
    welcomeName: "Bienvenido de nuevo, {name}",
    subtitleNamed: "Esto es lo que sucede con las compras mayoristas de {name}.",
    subtitle: "Esto es lo que sucede con tus compras mayoristas.",
    browseTitle: "Explorar productos",
    browseDesc: "Busca en el marketplace, compara precios y cantidades mínimas.",
    ordersTitle: "Mis pedidos",
    ordersDesc: "Sigue tus pedidos, descarga PDFs y revisa el estado de pago.",
    suppliersTitle: "Mis proveedores",
    suppliersDescCount: "{n} proveedores aprobados con precios especiales.",
    suppliersDescEmpty:
      "Solicita precios especiales a los mayoristas con los que trabajas.",
    recentOrders: "Pedidos recientes",
    noOrdersTitle: "Aún no hay pedidos",
    noOrdersDesc:
      "Empieza explorando el marketplace. Los productos públicos se pueden comprar directamente; para mejores precios, solicita precios de cliente a un mayorista.",
    browseCta: "Explorar marketplace",
    payTitle: "Pagos y crédito",
    payDue: "{amount} pendiente",
    payNoBalance: "Todo al día — sin saldo pendiente",
    payHeader: "Saldo pendiente por proveedor",
    payNoInvoices: "Sin facturas pendientes. Todo está al día.",
  },

  discover: {
    title: "Buscar proveedores",
    desc: "Explora todos los mayoristas y su catálogo. Los precios de cliente se muestran solo después de ser su cliente.",
    enter: "Ver tienda",
    notCustomer: "Aún no eres cliente — solicita precio para ver precios",
    searchPlaceholder: "Busca por mayorista o persona de contacto…",
    filterRegion: "Región:",
    searchBtn: "Buscar",
    noResults: "No se encontraron mayoristas",
    categoryLine: "Líneas principales: ",
  },

  browse: {
    title: "Marketplace",
    desc: "Descubre productos mayoristas de proveedores de toda América Latina.",
    searchPlaceholder: "Busca productos o categorías — describe lo que necesitas…",
    sortDefault: "Recomendados",
    sortPriceAsc: "Precio: menor a mayor",
    sortPriceDesc: "Precio: mayor a menor",
    similarTitle: "Sin coincidencia exacta — alternativas similares",
    similarDesc: "Revisa si alguno encaja o prueba otra palabra.",
    category: "Categoría",
    noProductsTitle: "No se encontraron productos",
    noProductsDesc: "Ajusta tu búsqueda o el filtro de categoría.",
    requestPricing: "Solicitar precio",
    yourPrice: "Precio exclusivo",
  },

  product: {
    minOrder: "Cantidad mínima",
    availability: "Disponibilidad",
    publicPrice: "Precio público",
    customerPrice: "Precio de cliente",
    paymentTerms: "Términos de pago",
    addToOrder: "Agregar al carrito",
    added: "Agregado ✓",
    viewSupplier: "Ver proveedor →",
    description: "Descripción",
    requestPricing: "Solicitar precio de cliente",
    reapply: "Volver a solicitar precio de cliente",
    pendingBadge: "Solicitud de precio pendiente",
    approvedBadge: "Cliente aprobado — tu precio está visible",
    rejectedMsg:
      "Tu solicitud de precio fue rechazada. Puedes volver a solicitarla.",
    pendingMsg:
      "Tu solicitud está siendo revisada por el mayorista.",
    customerOnlyMsg:
      "Los precios de este producto están disponibles para clientes aprobados.",
    approvalHint:
      "Una vez aprobado, verás aquí los precios exclusivos para clientes.",
    notPurchasable: "Solicita precio de cliente para comprar",
    quantity: "Cantidad",
  },

  suppliers: {
    title: "Mis proveedores",
    desc: "Mayoristas con los que trabajas. Cada relación se gestiona de forma independiente.",
    emptyTitle: "Aún no hay proveedores",
    partnerSince: "Socio desde",
    emptyDesc:
      "Explora el marketplace y solicita precios a los mayoristas para empezar a trabajar juntos.",
    productsCount: "{n} productos",
    approved: "Aprobado",
    pending: "Pendiente",
    rejected: "Rechazado",
    approvedCustomer: "Cliente aprobado",
    requestPending: "Solicitud pendiente",
    requestDeclined: "Solicitud rechazada",
    productsTitle: "Productos",
    newArrivals: "Novedades",
    allProducts: "Todos los productos",
    customerOnly: "Solo clientes",
    messagesTitle: "Mensajes",
    chatHint: "Envía fotos, PDF y más para comunicarte mejor.",
    openChat: "Abrir chat",
    minOrderValue: "Valor mínimo de pedido",
    count: "{n} proveedores",
    searchPlaceholder: "Buscar proveedores o chats…",
    emptySearch: "Sin proveedores o chats coincidentes",
  },

  orders: {
    title: "Pedidos",
    desc: "Sigue tus pedidos de todos los proveedores. Genera PDFs y revisa el estado de pago.",
    emptyTitle: "Aún no hay pedidos",
    emptyDesc:
      "Agrega productos a un pedido desde el marketplace y envíalo aquí.",
    orderNumber: "Pedido",
    suppliers: "Mis proveedores",
    discover: "Buscar proveedores",
    items: "Artículos",
    placed: "Realizado el {date}",
    orderPdf: "PDF del pedido",
    supplierTotal: "Total del proveedor",
    masterDesc: "{n} proveedores · un pedido maestro",
    notes: "Notas:",
    backToOrders: "Volver a pedidos",
    cancel: "Cancelar pedido",
    errCancel: "Solo se pueden cancelar pedidos pendientes de confirmación",
    errNotFound: "Pedido no encontrado",
    payNow: "Pagar ahora",
    paymentDue: "Pago pendiente",
    count: "{n} pedidos",
    searchOrders: "Buscar n.º pedido / proveedor / tienda…",
    noMatchOrders: "No se encontraron pedidos",
    dateFrom: "Desde",
    dateTo: "Hasta",
    resetFilter: "Restablecer",
  },

  cart: {
    title: "Tu pedido",
    desc: "Revisa los artículos y envía tu pedido. Los productos de distintos proveedores se agrupan automáticamente.",
    emptyTitle: "Tu pedido está vacío",
    emptyDesc: "Explora el marketplace y agrega productos a tu pedido.",
    placeOrder: "Enviar pedido",
    placing: "Enviando…",
    errEmpty: "El carrito está vacío",
    errUnavailable: "Un producto de tu carrito ya no está disponible",
    errNotPurchasable:
      "{name} no se puede comprar — solicita precio de cliente primero",
    errMoq: "{name}: la cantidad mínima es de {n} unidades",
    errStock: "{name}: solo hay {n} unidades en stock",
    errMinOrder: "{supplier} requiere un valor mínimo de pedido de {amount}",
    minOrder: "Pedido mín.",
    cartTitle: "Carrito",
    itemsCount: "{n} artículos",
    subtotal: "Subtotal",
    inStockCount: "En stock: {n}",
    checkoutNote: "Se aplica su precio de cliente. Envío e impuestos no incluidos; el importe final lo confirma el mayorista.",
    addedToast: "Añadido al carrito",
    viewCart: "Ver carrito",
    continueShopping: "Seguir comprando",
    goCheckout: "Finalizar compra",
    clearCart: "Vaciar carrito",
    clearConfirm: "¿Vaciar todo el carrito?",
    cartTotal: "Total",
    checkoutBack: "Volver al proveedor anterior para seguir comprando",
    submittedTitle: "Pedido enviado",
    submittedDesc: "Tu pedido fue enviado al proveedor y está pendiente de confirmación.",
    viewOrder: "Ver pedido",
    removeConfirm: "¿Eliminar este artículo del carrito?",
  },

  retailerAccount: {
    title: "Pagos y Cuentas",
    desc: "Controla lo que debes, registra pagos y revisa el historial.",
    totalOutstanding: "Total pendiente",
    searchBalances: "Buscar proveedor / n.º factura…",
    openInvoices: "Facturas abiertas",
    paymentsRecorded: "Pagos registrados",
    paymentMethod: "Método",
    searchInvoices: "Buscar n.º factura / proveedor / importe…",
    searchPayments: "Buscar proveedor / n.º pedido / importe…",
    balanceBySupplier: "Saldo por proveedor",
    noBalance: "Sin saldo pendiente.",
    wholesaler: "Mayorista",
    outstanding: "Pendiente",
    invoices: "Facturas",
    invoice: "Factura",
    due: "Vence",
    recordPayment: "Registrar un pago",
    payTo: "Pagar a",
    selectWs: "Selecciona mayorista…",
    wsOption: "{name} (pendiente {amount})",
    amountUsd: "Monto (USD)",
    method: "Método",
    bankTransfer: "Transferencia bancaria",
    cash: "Efectivo",
    card: "Tarjeta",
    other: "Otro",
    notes: "Notas",
    notesPlaceholder: "Referencia opcional…",
    recordBtn: "Registrar pago",
    pendingNote: "Los pagos quedan pendientes hasta que el mayorista los confirme.",
    recordedMsg:
      "Pago registrado — el mayorista lo confirmará en breve.",
    paymentHistory: "Historial de pagos",
    noPayments: "Aún no hay pagos registrados.",
    confirmed: "Confirmado",
    failed: "Fallido",
    pending: "Pendiente",
    pdf: "PDF",
    paid: "Pagado",
  },

  wholesalerHome: {
    processOrders: "Procesar pedidos",
    processOrdersDesc: "Revisa nuevos pedidos y confirma",
    recentActivity: "Actividad reciente",
    noActivity: "Sin actividad aún",
    title: "Panel",
    desc: "Resumen de tu negocio mayorista.",
    attention: "¿Qué necesita tu atención?",
    newOrders: "Nuevos pedidos por revisar",
    customerRequests: "Solicitudes de clientes",
    outOfStock: "Productos agotados",
    allCaughtUp: "Todo al día — nada necesita tu atención ahora.",
    overview: "Resumen",
    receivables: "Cuentas por cobrar",
    activeProducts: "Productos activos",
    activeCustomers: "Clientes activos",
    awaitingReview: "Pedidos por revisar",
    addProduct: "Agregar producto",
    addProductDesc: "Sube nuevos códigos a tu catálogo",
    reviewRequests: "Revisar solicitudes",
    reviewRequestsCount: "{n} minoristas esperando aprobación",
    noPending: "Sin solicitudes pendientes",
  },

  wsProducts: {
    title: "Productos",
    countDesc: "{n} productos en tu catálogo.",
    stdNote: "Formatos de producto estandarizados (unidades, espec., códigos)",
    selectNone: "Selecciona productos primero",
    bulkSelectHint: "Marca productos para gestión masiva",
    activate: "Activar",
    deactivate: "Desactivar",
    bulkSetPrice: "Fijar precio",
    bulkDelete: "Eliminar",
    bulkDeleteConfirm: "¿Eliminar los productos seleccionados? Los vinculados a pedidos/cotizaciones se desactivarán.",
    bulkPricePrompt: "Introduce el nuevo precio público:",
    errPrice: "Precio inválido",
    importCsv: "Importar CSV",
    addProduct: "Agregar producto",
    emptyTitle: "Aún no hay productos",
    emptyDesc:
      "Agrega tu primer producto o importa un CSV para empezar a vender.",
    product: "Producto",
    category: "Categoría",
    price: "Precio",
    moq: "Cant. mín.",
    stock: "Stock",

    mode: "Modo",
    edit: "Editar",
    deleteProduct: "Eliminar",
    bulkUpload: "Alta masiva inteligente",
    bulkTitle: "Generar productos en masa",
    bulkHint:
      "Sube fotos de productos, listas de precios, Excel o capturas: la IA lee y rellena los datos; revisa y crea en lote. Todo editable.",
    dupWarn: "Posible duplicado en tu catálogo: {name}",
    chooseFiles: "Elegir archivos / Añadir más",
    addMore: "Añadir más",
    startRecognize: "Iniciar reconocimiento IA",
    uploadHintNew:
      "Sube en lote / varias veces imágenes, PDF, Excel; toca \u201cIniciar IA\u201d cuando estén todos",
    fileCount: "{n} archivos añadidos",
    preview: "Vista previa",
    clearFiles: "Vaciar",
    fileImage: "Imagen",
    filePdf: "PDF",
    fileExcel: "Hoja",
    recognizingNow: "IA leyendo archivo {i}/{n}",
    recognizeAll: "Reconocidos — revisa y crea en lote",
    aiFailed: "No reconocido, rellena manualmente",
    backToFiles: "← Volver a archivos",
    aiDetecting: "La IA lee la imagen…",
    aiDone: "La IA completó los datos: editable",
    dupSkipped: "Omitido (no se añade)",
    skipDup: "Omitir esta",
    mergeDup: "Mantener ambas (añadir igual)",
    selectAll: "Seleccionar todo",
    deselectAll: "Deseleccionar todo",
    deleteSelected: "Eliminar seleccionados",
    createSelected: "Crear seleccionados",
    uploadingPhotos: "Subiendo fotos…",
    bulkCreated: "Se crearon {n} productos",
    errQuota: "Cuota mensual de productos alcanzada. Actualiza a Plus para más.",
    deleteConfirm:
      "¿Eliminar este producto? Si está referenciado por pedidos o precios de cliente, se desactivará y ocultará de la lista (se conserva el historial).",
    deleteNotFound: "Producto no encontrado o sin permiso",
    csvTitle: "Importación masiva",
    csvHint: "Columnas CSV: name, sku, price, moq, stock, sellingmode, imageUrl, description",
    csvDone: "Importación completada.",
    csvUpload: "Subir e importar",
    csvErrNoFile: "No se proporcionó archivo CSV",
    csvErrFormat:
      "El CSV debe tener una fila de encabezado y al menos un producto",
    csvErrColumns: 'El CSV debe incluir las columnas "name" y "sku"',
    csvErrEmpty:
      "No se importaron productos nuevos (SKUs ya existentes o filas inválidas)",
  },

  productForm: {
    costPrice: "Precio de costo",
    lowStockThreshold: "Alerta de stock bajo en",
    lowStockHint: "Avisar cuando el stock baje de esta cantidad (vacío = off)",
    boxSize: "Tamaño de caja",
    boxSizeHint: "p. ej. 24/caja, 50 pzas/caja — los compradores estiman el flete",
    boxSizeRequired: "El tamaño de caja es obligatorio",
    showBoxSizeToBuyer: "Mostrar caja a compradores (recomendado)",
    multilangNames: "Nombres multilingües (el comprador ve su idioma; vacío usa el nombre principal)",
    nameZh: "中文品名（选填）",
    nameEn: "English name (optional)",
    nameEs: "Nombre en español (opcional)",
    addTitle: "Agregar producto",
    addDesc: "Crea un producto con precio, cantidad mínima e inventario.",
    editTitle: "Editar producto",
    name: "Nombre del producto *",
    sku: "Código *",
    category: "Categoría",
    uncategorized: "Sin categoría",
    description: "Descripción",
    imageUrl: "URL de imagen",
    upload: "Subir",
    publicPrice: "Precio público",
    moq: "Cant. mín.",
    stock: "Stock (almacén principal)",
    stockHint: "Indica cantidad; 0 = sin stock",

    sellingMode: "Modo de venta",
    modeBoth: "Público + precio de cliente (recomendado)",
    modePublic: "Solo público",
    modeCustomer: "Solo clientes aprobados",
    createBtn: "Crear producto",
    errName: "El nombre del producto es obligatorio",
    errSku: "El código es obligatorio",
    errSkuExists: 'El código "{sku}" ya existe',
    errCreate: "No se pudo crear el producto",
    errNotFound: "Producto no encontrado",
    backToProducts: "Volver a productos",
  },

  wsOrders: {
    shareOrder: "Compartir por WhatsApp",
    title: "Pedidos",
    desc: "Revisa y despacha los pedidos recibidos.",
    emptyTitle: "Aún no hay pedidos",
    emptyDesc: "Cuando los minoristas envíen pedidos, aparecerán aquí.",
    customer: "Cliente",
    confirm: "Confirmar pedido",
    preparing: "Marcar en preparación",
    ready: "Marcar listo",
    complete: "Completar",
    cancel: "Cancelar pedido",
    errNotFound: "Pedido no encontrado",
    errTransition: "No se puede pasar de {from} a {to}",
    errStock: "Stock insuficiente: {name} ({stock} disponibles, se necesitan {needed})",
    deleteOrder: "Eliminar",
    searchOrders: "Buscar n.º pedido / tienda / contacto…",
    noMatchOrders: "No se encontraron pedidos",
    selAll: "Seleccionar todos",
    clearSel: "Limpiar selección",
    selectedCount: "{n} pedidos seleccionados",
    exportSelected: "Exportar PDF seleccionados",
    delSelected: "Eliminar seleccionados",
    batchDelTitle: "¿Eliminar pedidos seleccionados?",
    batchDelWarn:
      "{n} pedidos se ocultarán en tu portal; los datos quedan retenidos en el panel admin (borrado suave, recuperable).",
    batchDelActiveWarn:
      "Algunos están COMPLETADOS / PENDIENTES: se ocultarán también sus registros de conciliación.",
    cancelBtn: "Cancelar",
    confirmDelBtn: "Sí, eliminar",
    estMargin: "Margen est.",
    removedTip: "{n} pedidos eliminados (retenidos en admin)",
    dateFrom: "Desde",
    dateTo: "Hasta",
    resetFilter: "Restablecer",
    deleteTitle: "Eliminar pedido — no se puede deshacer",
    deleteConfirm:
      "Eliminar este pedido de proveedor quita el pedido y sus líneas de forma permanente. Si el pedido principal del comprador no tiene otros pedidos de proveedor, también se cancelará. Los pedidos con factura o pagos recibidos no pueden eliminarse — cancélalo primero.",
    errHasInvoice: "Este pedido tiene una factura y no puede eliminarse",
    errHasPayment: "Este pedido tiene pagos recibidos y no puede eliminarse",
    errActive: "Este pedido está en proceso; cancélalo antes de eliminar",
    unitPrice: "Precio unitario",
  },

  wsCustomers: {
    title: "Clientes",
    inviteCustomer: "Invitar cliente",
    inviteCopied: "Enlace de invitación copiado — compártelo con minoristas para que soliciten colaborar",
    searchPlaceholder: "Busca por empresa o contacto…",
    desc: "Aprueba solicitudes, configura términos y gestiona precios por cliente.",
    requests: "Solicitudes de clientes ({n})",
    requestedAt: "solicitado el {date}",
    approve: "Aprobar",
    reject: "Rechazar",
    approved: "Clientes aprobados ({n})",
    noApproved: "Aún no hay clientes aprobados.",
    tier: "Nivel",
    terms: "Términos",
    creditLimit: "Límite de crédito",
    pricesSet: "Precios configurados",
    declined: "Solicitudes rechazadas ({n})",
    credit: "Crédito {amount}",
    backToCustomers: "Volver a clientes",
    customerSince: "Cliente desde el {date}",
    chatWith: "Chatear con {name}",
    statementPdf: "Estado de cuenta PDF",
    customerTerms: "Términos del cliente",
    termsHint:
      "El nivel, los términos de pago y el límite de crédito aplican solo a este cliente.",
    customerTier: "Nivel del cliente",
    termsFull: "Pago completo (COD — sin crédito)",
    termsNet: "Crédito Net {n} días",
    termsHintNone: "Sin términos de crédito — pago completo requerido",
    paymentTerms: "Términos de pago",
    termsExample: "ej. NET30",
    creditUsd: "Límite de crédito",
    creditExample: "ej. 5000",
    saveTerms: "Guardar términos",
    saved: "Guardado ✓",
    customPricing: "Precios específicos por cliente",
    pricingHint:
      "Configura un precio personalizado por producto. Los clientes aprobados verán estos precios en lugar del precio público.",
    publicPrice: "Precio público",
    customerPrice: "Precio de cliente",
    price: "Precio",
    errNotApproved: "Cliente no aprobado",
    errProduct: "Producto no encontrado",
    errTier: "Nivel inválido",
    errNotFound: "Relación no encontrada",
    errHasHistory: "Este cliente tiene pedidos o facturas y no puede eliminarse (se conserva el registro financiero)",
    deleteCustomer: "Eliminar cliente",
    deleteCustomerHint: "Haz clic dos veces para confirmar",
    deleteConfirm: "Eliminar este cliente borra sus precios personalizados y perderá el acceso a tus precios de cliente. No se puede deshacer. Los clientes con pedidos o facturas no pueden eliminarse.",
  },

  wsReports: {
    title: "Informe mensual",
    desc: "Resumen mensual de ventas, costo y margen con exportación CSV.",
    month: "Mes",
    exportCsv: "Exportar CSV",
    sales: "Ventas",
    cost: "Costo",
    margin: "Margen",
    orderCount: "Pedidos",
    orderCol: "Pedido",
    dateCol: "Fecha",
    customerCol: "Cliente",
    emptyReport: "Sin pedidos completados este mes",
    marginNote: "Costo estimado con el precio de costo actual del producto.",
  },
  wsAccount: {
    businessProfile: "Perfil de tienda",
    businessProfileDesc: "Información visible para los minoristas: se actualiza en todos los puntos.",
    tradeName: "Nombre de tienda",
    tradeNameHint: "p. ej. Caribbean Wholesale",
    legalName: "Razón social",
    phone: "Teléfono",
    address: "Dirección",
    taxId: "NIF",
    profileSaved: "Perfil de tienda guardado",
    title: "Cuentas por cobrar",
    desc: "Controla los saldos pendientes y confirma los pagos recibidos.",
    logo: "Logo de tienda",
    uploadLogo: "Subir logo",
    logoHint: "Se muestra en tu tienda para que los minoristas te reconozcan.",
    planTitle: "Plan de suscripción",
    planFree: "Plan gratuito",
    planPlus: "Plus",
    upgrade: "Actualizar a Plus",
    upgradeComingSoon: "Los pagos aún no están disponibles — próximamente",
    planUpgradeHint: "Plus desbloquea mayores límites de subida y productos ilimitados.",
    totalOutstanding: "Total por cobrar",
    pendingConfirmations: "Pagos por confirmar",
    overdueInvoices: "Facturas vencidas",
    awaiting: "Pendientes de confirmación",
    confirmReceived: "Confirmar recibo",
    confirmName: "Tu nombre (persona que confirma)",
    confirmedBy: "Confirmado por {name}",
    customerOutstanding: "Pendiente por cliente",
    allCaughtUp: "Todo al día — sin facturas pendientes.",
    paymentHistory: "Historial de pagos",
    noPayments: "Aún no hay pagos recibidos.",
    errNotFound: "Pago no encontrado",
    errProcessed: "El pago ya fue procesado",
    policyTitle: "Política de pedidos y pagos",
    policyDesc: "Define el valor mínimo de pedido para todos los compradores (pedidos de pago completo y a crédito). Déjalo vacío si no hay mínimo.",
    minOrderAmount: "Valor mínimo de pedido",
    minOrderHint: "ej. 500",
    policySaved: "Política actualizada ✓",
  },

  admin: {
    title: "Administración",
    desc: "Consola de gestión de la plataforma.",
    users: "Usuarios",
    wholesalers: "Mayoristas",
    retailers: "Minoristas",
    products: "Productos",
    orders: "Pedidos",
    countries: "Países",
    recentUsers: "Usuarios recientes",
    usersSearch: "Buscar usuarios",
    usersSearchHint: "Busca por nombre o correo",
    countriesList: "Países y regiones",
    noUsers: "No se encontraron usuarios",
    noRegions: "Sin registros",
    viewCountry: "Ver",
    companies: "{n} empresas",
    pending: "Pendiente",
    manageBtn: "Gestionar",

    name: "Nombre",
    email: "Correo",
    role: "Rol",
    business: "Negocio",
    joined: "Registro",
    viewAll: "Ver todo",
    viewDetails: "Ver detalles",
    allOrders: "Todos",
    normalOnly: "Pedidos activos",
    deletedOnly: "Eliminados (ocultos del portal)",
    deletedTag: "Eliminado",
    operator: "Eliminado por",
    deleteTime: "Eliminado el",
    ip: "IP",
    restore: "Restaurar",
    restoredTip: "Pedido restaurado",
    orderCol: "Pedido",
    buyerCol: "Comprador",
    totalCol: "Total",
    emptyOrders: "Sin pedidos",
    connect: "Conectar",
    connectRetailer: "Conectar minorista",
    selectRetailer: "Seleccionar minorista…",
    relationships: "Relaciones de clientes",
    noRelationships: "Aún no hay relaciones",
    connected: "Conectado",
    connectSuccess: "Conectado",
    country: "País",
    productsCount: "{n} productos",
    sku: "Código",
    console: "Consola Admin",
  },

  messages: {
    title: "Mensajes",
    expand: "Expandir",
    collapse: "Contraer",
    empty: "Aún no hay mensajes — envía el primero.",
    you: "Tú",
    supplier: "Proveedor",
    placeholder: "Escribe un mensaje…",
    attach: "Adjuntar",
    sendProduct: "Enviar producto",
    sendOrder: "Enviar pedido",
    openLink: "Abrir",
    noCards: "Nada para compartir aún",

    errEmpty: "El mensaje no puede estar vacío",
    errSignedOut: "No has iniciado sesión",
    errWsNotFound: "Mayorista no encontrado",
    errForbidden: "Prohibido",
  },

  statusOrder: {
    DRAFT: "Borrador",
    SUBMITTED: "Enviado",
    CONFIRMED: "Confirmado",
    PREPARING: "En preparación",
    READY: "Listo",
    COMPLETED: "Completado",
    CANCELLED: "Cancelado",
  },

  statusRelationship: {
    PENDING: "Pendiente",
    APPROVED: "Aprobado",
    REJECTED: "Rechazado",
  },

  statusInvoice: {
    UNPAID: "Sin pagar",
    PARTIALLY_PAID: "Pago parcial",
    PAID: "Pagado",
    OVERDUE: "Vencida",
    CANCELLED: "Cancelada",
  },

  statusPayment: {
    PENDING: "Pendiente",
    RECEIVED: "Recibido",
    FAILED: "Fallido",
  },

  statusSellingMode: {
    PUBLIC: "Solo público",
    CUSTOMER_ONLY: "Solo clientes",
    BOTH: "Público + precio cliente",
  },

  statusTier: {
    STANDARD: "Estándar",
    VIP: "VIP",
    GOLD: "Oro",
    VOLUME: "Volumen",
  },

  pdf: {
    brand: "Latam B2B",
    brandTag: "Plataforma de comercio mayorista",
    purchaseOrder: "ORDEN DE COMPRA",
    invoice: "FACTURA",
    statement: "ESTADO DE CUENTA",
    asOf: "Al {date}",
    date: "Fecha",
    due: "Vence",
    status: "Estado",
    fromSupplier: "De (Proveedor)",
    billTo: "Facturar a",
    buyer: "Comprador",
    supplier: "Proveedor {n}",
    product: "Producto",
    sku: "Código",
    unitPrice: "Precio unit.",
    qty: "Cant.",
    subtotal: "Subtotal",
    amount: "Monto",
    discount: "Descuento",
    shipping: "Envío",
    supplierTotal: "Total del proveedor",
    grandTotal: "Total general",
    orderLabel: "Pedido #{n}",
    orderItems: "Artículos del pedido",
    notes: "Notas:",
    totalDue: "Total a pagar",
    invoices: "Facturas",
    invoiceNumber: "Factura",
    seller: "Vendedor",
    item: "Artículo",
    total: "Total",
    generatedAt: "Generado {d}",
    outstandingBalance: "Saldo pendiente",
    footerOrder: "Latam B2B Plataforma de comercio mayorista",
    footerInvoice: "Se aplican los términos de pago acordados. Gracias por tu compra.",
    footerStatement: "Por favor realiza el pago según los términos acordados.",
    customer: "Cliente",
  },
};

export default es;
